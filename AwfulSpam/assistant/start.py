...
from AwfulSpam import ALIVE_PIC, Roodversion, OWNER_ID
from AwfulSpam import STRING, STRING2, STRING3, STRING4, STRING5 , STRING6, STRING7, STRING8, STRING9, STRING10, STRING11, STRING12, STRING13, STRING14, STRING15, STRING16, STRING17, STRING18, STRING19, STRING20, STRING21, STRING22, STRING23, STRING24, STRING25, STRING26, STRING27, STRING28, STRING29, STRING30, STRING31, STRING32, STRING33, STRING34, STRING35, STRING36, STRING37, STRING38, STRING39, STRING40
from telethon.tl.functions.users import GetFullUserRequest


btn = [
      [ 
      Button.url("Channel", "https://t.me/AwfulSpam"),
      Button.url("Support", "https://t.me/AwfulSupport")
      ],
      [
...
__Report Bugs At :__ **@AwfulSupport**
...
@Rood.on(events.NewMessage(pattern="[!/]start"))
async def start_all(event):
   ...
   if event.is_private:
      ...
      if event.sender_id == OWNER_ID or event.sender_id in DEV:
          await Rood.send_message(chat, start_owner, buttons=btn)
      elif event.sender_id in SUDO_USERS:
          await Rood.send_message(chat, start_sudo, buttons=btn)
      else:
          await Rood.send_message(chat, start_users, buttons=btn)
          return
   if event.is_group:
...
@Rood.on(events.callbackquery.CallbackQuery(data="help"))
async def Help_cmds_(event):
       await event.edit(HELP_TEXT, buttons=help_btn)


@Rood.on(events.callbackquery.CallbackQuery(data="admin"))
...
@Rood.on(events.callbackquery.CallbackQuery(data="zombies"))
async def _(event):
    await event.edit(CLEANER_HELP, buttons=[[Button.inline("« Bᴀᴄᴋ", data="help")]])


@Rood.on(events.callbackquery.CallbackQuery(data="locks"))
async def _(event):
    await event.edit(LOCKS_HELP, buttons=[[Button.inline("« Bᴀᴄᴋ", data="help")]])
 
 
@Rood.on(events.callbackquery.CallbackQuery(data="misc"))
async def _(event):
    await event.edit(MISC_HELP, buttons=[[Button.inline("« Bᴀᴄᴋ", data="help")]])


@Rood.on(events.callbackquery.CallbackQuery(data="pins"))
async def _(event):
    await event.edit(PINS_TEXT, buttons=[[Button.inline("« Bᴀᴄᴋ", data="help")]])


@Rood.on(events.callbackquery.CallbackQuery(data="purges"))
async def _(event):
...
@Rood.on(events.NewMessage(pattern="[!/]ping"))
async def Botping(e):
   if e.sender_id in SUDO_USERS or e.sender_id in DEV:
        ...
        pingop = f"࿊ Aωƒυll Sραм ࿊\n\n Pong : `{ms}` ᴍs \n Type : Private " 
        await fuk.edit(pingop)
        


@Rood.on(events.NewMessage(pattern="[!/]alive"))
async def alive(event):
   if event.sender_id in SUDO_USERS or event.sender_id in DEV:
        ...
        try:
           ...
           if STRING40:
              ids += 1 
           Caption = f"**࿊ Aωƒυll Sραм ࿊** \n\n"
           Caption += f"━━━━━━━━━ × ━━━━━━━━━┑\n"
           Caption += f"• **Master:** [{OWNER_ID}](tg://user?id={OWNER_ID}) \n"
           Caption += f"• **Active IDs:** `{ids}` \n"
           Caption += f"━━━━━━━━━ × ━━━━━━━━━┙\n"
           await Rood.send_file(event.chat_id, 
                                RIZ_PIC, 
                                caption=Caption, 
                                buttons=[
           [
           Button.url("Channel", "https://t.me/AwfulSpam"),
           Button.url("Support", "https://t.me/AwfulSpamSupport")
           ], 
           ], 
           )        
...
@Rood.on(events.NewMessage(pattern="[!/]restart"))
async def restart(event):
  if event.sender_id in DEV:
     text = "**Restarting**"
     await event.reply(text, parse_mode=None, link_preview=None )
     try:
        await Rood.disconnect()
     except Exception:
        pass
  ...
  if event.sender_id in SUDO_USERS:
        await event.reply("**This Cmd is For Full Sudo And Owner**")
