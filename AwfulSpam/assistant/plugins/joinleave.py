...
@Rood.on(events.NewMessage(pattern="[!/]join"))
async def join(event):
  if event.sender_id in DEV or event.sender_id in SUDO_USERS:
     ...
     async with Rood.conversation(event.chat_id) as rizx:
        await rizx.send_message("**Send Group Link or User-name To Join**")
        ...
        if grpp.startswith("https://t.me/") or grpp.startswith("@"):
            ...
            await rizx.send_message("Error! Send Group link or User-name")
            return
...
@Rood.on(events.NewMessage(pattern="[!/]pjoin"))
...
@Rood.on(events.NewMessage(pattern="[!/]leave"))
