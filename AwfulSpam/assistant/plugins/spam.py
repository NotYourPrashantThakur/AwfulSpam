...
from AwfulSpam import OWNER_ID, hl

SEND_COUNT ="**Send Counts**"
SEND_SPAM = "**Send spam message to spam**"
ABORT = "**All process Cancelled !**"

@Rood.on(events.NewMessage(pattern="[!/]spam"))
async def spam(event):
     if event.sender_id in DEV or event.sender_id in Sudo_USERS:
            await event.reply("**We'll Add This Cmd Soon.**")

@Rood.on(events.NewMessage(pattern="[!/]dmspam"))
async def dm_spam(event):
  if event.sender_id in DEV or event.sender_id in Sudo_USERS:
     if event.is_group:
         return await event.reply("**Use This Cmd In Inbox**")
     async with Rood.conversation(event.chat_id) as rizx:
        await rizx.send_message("**Send User-name or User-id to Start Dm Spam**")
        ...
        if username.startswith("@"):
            try:
               ...
               id = k.id
               if int(id) in RoodX:
                   return await rizx.send_message("He is AwfulSpam's Owner.")
               elif int(id) == OWNER_ID:
                   return await rizx.send_message("He is Owner Of This Bots.")
               elif int(id) in DEV:
                   return await rizx.send_message("He is Full Sudo User.")
               elif int(id) in Sudo_USERS:
                   return await rizx.send_message("He is Sudo User.")
               else:
                   fname = k.first_name
            ...
            await rizx.send_message("Error! User-name or User-id")
            return
...
@Rood.on(events.NewMessage(pattern="[!/]dm"))
async def dm(event):
  if event.sender_id in DEV or event.sender_id in Sudo_USERS:
     if event.is_group:
          return await event.reply("**Use This Cmd In Dm**")
     async with Rood.conversation(event.chat_id) as rizx:
        await rizx.send_message("**Send User-name or User-id to Start Dm Spam**")
        response = rizx.wait_event(events.NewMessage(chats=event.chat_id))
        res = await response
        ...
        if username.startswith("@"):
            ...
            await rizx.send_message("Error! Send User-name or User-id")
            return
        ...
        await asyncio.sleep(2)
        await Fukoff.edit(f"**Message Sent** \n\n__User__ : {fukname}\n__Message__ : `{message}`")
        try:
           if Riz: 
                    await Riz.send_message(id, message) 
           if Riz2: 
