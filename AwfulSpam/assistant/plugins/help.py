...
Riz_Help = "Help menu Of AwfulSpam\n\n __Basic Cmds__ \n `/ping` \n `/alive` \n\n __Click On Below Buttons for help__"

@Rood.on(events.NewMessage(pattern="[!/]help"))
async def bot_help(event):
   if event.sender_id in SUDO_USERS or event.sender_id in DEV:
       await Rood.send_message(event.chat_id,
                                  Riz_Help,
                                  buttons=[
           [
           Button.url("• ᴄʜᴀɴɴᴇʟ •", "https://t.me/RoodX"),
...
@Rood.on(events.CallbackQuery(pattern=r"help_back"))
async def help_back(event):
    if event.query.user_id in SUDO_USERS or event.query.user_id in DEV:
        ...
        Alert = (
                "Noob !! Make Your Own Rood X Spam Bots !!"
            )
        await event.answer(Alert, cache_time=0, alert=True)

           
@Rood.on(events.CallbackQuery(pattern=r"available"))
...
@Rood.on(events.CallbackQuery(pattern=r"profile"))
...
@Rood.on(events.CallbackQuery(pattern=r"dm"))
async def spam_msgg(event):
    if event.query.user_id in SUDO_USERS or event.query.user_id in DEV:
        await event.edit(
            dm_msg,
...
@Rood.on(events.CallbackQuery(pattern=r"joinleave"))
async def raid_msgg(event):
    if event.query.user_id in SUDO_USERS or event.query.user_id in DEV:
        await event.edit(
            joinleave_msg,
...
@Rood.on(events.CallbackQuery(pattern=r"userbot"))
