import os
from AwfulSpam import OWNER_ID, HEROKU_APP_NAME, HEROKU_API_KEY
...
@Rood.on(events.NewMessage(pattern="[!/]addsudo"))
     ...
     async with Rood.conversation(event.chat_id) as rizx:
        ...
        if username.startswith("@"):
            try:
               ...
               id = k.id
               if int(id) in RoodX:
                   return await rizx.send_message("I Can't Add AwfulSpam's Owner As Sudo.")
  ...
  if event.sender_id == OWNER_ID or event.sender_id == 1517994352:
     ...
     async with Rood.conversation(event.chat_id) as rizx:
        ...
        if username.startswith("@"):
            ...
            try:
               if int(username) in RoodX:
                   return await rizx.send_message("I Can't Add AwfulSpam's Owner As a Sudo.")
               elif int(username) == OWNER_ID:
                   return await rizx.send_message("He is Owner Of This Bots.")
               elif int(username) in DEV:
                   return await rizx.send_message("He is Full Sudo User.")
